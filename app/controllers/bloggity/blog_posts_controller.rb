module Bloggity
  class BlogPostsController < Bloggity::ApplicationController
    ...
    # ----------------------------------------------------------------------------------------------------------
  	...
  	def load_blog_post
  		load_blog
  		@blog_post = BlogPost.where("blog_id = ? AND (id = ? OR url_identifier = ?)", @blog_id, params[:id], params[:id]).first if params[:id]
  	end
