...
class BlogPostsControllerTest < ActionController::TestCase
	...
	def test_blog_show_normal
		...
		assert_equal assigns(:blog_posts).size, BlogPost.where(:blog_id => blog_post.blog_id, :is_complete => true).count
		assert_equal assigns(:blog_id), blog_post.blog_id
	end

	def test_blog_show_secondary_index
		blog_post = BlogPost.all[1]
	...
	def test_blog_no_show_incomplete
		blog_post = BlogPost.find_by(:is_complete => false)
		User.class_eval do
